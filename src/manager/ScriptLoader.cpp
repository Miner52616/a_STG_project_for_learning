    ...
    lua_(lua)
{
    ...
    registerPhase("MidPhase",
        [this](const sol::table& phasescript)
        {
            ...
            std::unique_ptr<MidPhase> phase=PEBfactory_.createPhase(phasescript);
...
void ScriptLoader::registerPhase(const std::string& name,PhaseCreator creator)
{
...
void PEBFactory::registerPhase(const std::string& name,PhaseCreator creator)
{
    phaseregistry_[name]=creator;
}
...
std::unique_ptr<Phase> PEBFactory::createPhase(const sol::table& phasescript)
{
    std::string type=phasescript["type"];
